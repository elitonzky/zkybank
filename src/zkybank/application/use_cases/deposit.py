...
class DepositUseCase:
    ...
    def execute(self, command: DepositCommand) -> TransactionResult:
        ...
        for attempt in range(3):
            try:
                with self._uow as uow:
                    ...
                    self._record_deposit(uow, account, amount)

                    uow.accounts.save(account)
                    uow.commit()

                self._log_deposit_succeeded(account, amount, attempt + 1)
                ...
                logger.warning(
                    "deposit_concurrency_conflict_retry",
                    extra={"account_number": account_number.value, "attempt": attempt + 1},
                )
                continue

        raise ConcurrencyConflictError("Deposit failed after retries")
    ...
    def _record_deposit(self, uow: UnitOfWork, account: Account, amount: Money) -> None:
        uow.ledger.save(
            LedgerEntry.create(
    ...
    def _log_deposit_succeeded(self, account: Account, amount: Money, attempt: int) -> None:
        logger.info(
            "deposit_succeeded",
            extra={
                "account_number": account.account_number.value,
